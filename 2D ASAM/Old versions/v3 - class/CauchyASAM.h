	...
	for (idx=0;idx<NUMSAM;idx++) {
		...
		if (denchy != 0.0) {
			for (i=0;i<NUMPAT;i++) {
				...
				xmd = (x[idx]-means(i,0))/disps(i,0); 
				ymd = (y[idx]-means(i,1))/disps(i,1);
				cx = 1.0/(1.0 + xmd*xmd);
				cy = 1.0/(1.0 + ymd*ymd);
				...
				means(i,0) -= mu_m*dEdmx/adaptCounter; 
				means(i,1) -= mu_m*dEdmy/adaptCounter;
				disps(i,0) -= mu_d*dEddx/adaptCounter; 
				disps(i,1) -= mu_d*dEddy/adaptCounter;


				/*disps(i,0) = abs(disps(i,0)); 
				disps(i,1) = abs(disps(i,1));*/
			}
		}
	}
	ASAM::Learn();
